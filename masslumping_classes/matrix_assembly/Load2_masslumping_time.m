function F = Load2_masslumping_time(T,fnf,fnk,g,h,t)

% F = Load2_masslumping_time(T,fnf,fnk,g,h,t)

%   Assembles the load vector for the wave problem

%   Here we consider a general case where f(x,y,t) can't be factorize
%   in spatial part * time part. If you can, consider using
%   Load2_masslumping for spatial times the evaluation in time at each step
%
%   The right-hand-side function f(x,y,t) must be implemented
...
%   Modified starting from "Load2.m" in "Understanding and Implementing the Finite
...
if ~isempty(fnf) || ~isempty(g)
   ...
   [qpts, qwts] = qpts_qwts(d);
   npts = length(qwts);            %quadratic mass lumping quadrature nodes

   % Extract the interpolation nodes from the reference triangle:

   inodes=getNodes_masslumping(TR,1);   %mass_lumping functions
...
if ~isempty(fnf)
   [V, ~, ~] = shape_functions_masslumping(qpts);
end
...
if ~isempty(g)
   [~,Vs,Vt]= shape_functions_masslumping(qpts);
end
