function [m,elerrs]=H1EnergyNormErr_dunavant(T,fnk,fnvx,fnvy,U,g,qo)

% m=H1EnergyNormErr_dunavant(T,fnk,fnvx,fnvy,U,g,qo)
%   (seminorm)
...
nodes=getNodes_masslumping(TR,1);
%V=EvalNodalBasisFcns_masslumping(nodes,qpts); %V=eval_matrix_prova(nodes,qpts);
                                  %%V=EvalNodalBasisFcns_prova(nodes,qpts);
%[Vs,Vt]=EvalNodalBasisGrads(nodes,qpts); 
[V,Vs,Vt]=shape_functions_masslumping(nodes, qpts);
