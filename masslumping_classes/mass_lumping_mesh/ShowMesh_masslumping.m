function ShowMesh_masslumping(T,flags,c)

% ShowMesh_masslumping(T,flags,c)
%
...
% Modified starting from ShowMesh of "Understanding and Implementing the Finite
...
X=T.Nodes(:,1);
Y=T.Nodes(:,2);
[tris,tris1]=getTriNodeIndices3_masslumping(T);
tris=tris(:,1:d:2*d+1);
...
if any(flags)
   ...
   tris=getTriNodeIndices_masslumping(T);
...
function [ElList,ElList1]=getTriNodeIndices3_masslumping(T)
...
for k=1:Nt
   [t1,t2,indices]=getNodes_masslumping(T,k);
